<?php
class GhoSter_VietnameseCheckout_Model_Observer
{
    ...
    {
        if (! Mage::getStoreConfigFlag('vietnamesecheckout/cart_settings/enabled')) {
            return;
        }
        ...
        if (!($country = $this->_getCoreSession()->getAutoShippingCountry())) {
            $country = Mage::getStoreConfig('vietnamesecheckout/cart_settings/country_id');
    ...
    public function addShipping(Varien_Event_Observer $observer)
    {
        ...
        if (count($rates)) {
            ...
            foreach($topRates as $topRate) {
                ...
                if (in_array($topRate->getCarrier(), explode(',', Mage::getStoreConfig('vietnamesecheckout/cart_settings/ignore_shipping_methods')))) {
                    continue;
                }
    ...
    public function beforeBlockToHtml($observer)
    {
        ...
        if (! Mage::getStoreConfigFlag('vietnamesecheckout/cart_settings/show_country_selection_in_cart')) {
            return;
        }
        ...
        if ($block instanceof Mage_Tax_Block_Checkout_Shipping) {
            ...
            $block->getTotal()->setTitle(
                $block->getLayout()->createBlock('ghoster_vietnamesecheckout/country', 'checkout_cart_country_select')->toHtml()
            );
        }
    }
